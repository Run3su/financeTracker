...
import { Home, ShoppingBag, Car, Sparkles, Plus } from 'lucide-react';
...
const CATEGORIES = [
  { id: 'Housing', label: 'Housing', icon: Home, activeBg: 'bg-indigo-600', color: 'text-indigo-600' },
  { id: 'Food', label: 'Food', icon: ShoppingBag, activeBg: 'bg-violet-600', color: 'text-violet-600' },
  { id: 'Transport', label: 'Transport', icon: Car, activeBg: 'bg-pink-600', color: 'text-pink-600' },
  { id: 'Extras', label: 'Extras', icon: Sparkles, activeBg: 'bg-emerald-600', color: 'text-emerald-600' },
];

const AddExpensePanel: React.FC<AddExpensePanelProps> = ({ onAddExpense }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (selected && amount) {
      onAddExpense(selected, Number(amount));
      setAmount('');
      setSelected(null);
    }
  };
  ...
  return (
    <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 animate-fade-in mt-6">
        <div className="flex items-center justify-between mb-4 px-1">
            <h2 className="text-lg font-bold text-slate-800">Quick Expense</h2>
            <span className="text-xs text-slate-400 font-medium">Add new spending</span>
        </div>
      
      {/* Category Selector */}
      <div className="flex justify-between gap-3 mb-6 overflow-x-auto pb-2 no-scrollbar">
        {CATEGORIES.map((cat) => {
          const Icon = cat.icon;
      ...
      <form onSubmit={handleSubmit} className="flex items-center gap-3 bg-slate-50 p-2 pr-2 pl-5 rounded-2xl border border-slate-100 focus-within:border-blue-200 focus-within:ring-4 focus-within:ring-blue-500/10 transition-all">
        ...
        <button
          ...
          className="bg-slate-900 disabled:bg-slate-200 disabled:text-slate-400 text-white w-12 h-12 rounded-xl flex items-center justify-center shadow-lg shadow-slate-900/20 disabled:shadow-none hover:scale-105 active:scale-95 transition-all"
        >
